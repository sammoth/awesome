...
local function insert_wrapper(handler, c, wrapper, in_place_of)
    ...
    if in_place_of then
        ...
        wrapper:emit_signal_recursive("added", c)
    end
end
...
local function remove_wrapper(handler, c, wrapper)
    ...
    -- Emit the signal while it is still in the hierarchy
    wrapper:emit_signal_recursive("will_remove", other_c)
...
local function get_client_differential(self)
    ...
    for _,c in ipairs(clients) do
        ...
        local is_tiled = aw_client.property.get(c, "floating") ~= true

        if is_tiled and (not self.client_to_wrapper[c]) and (not c.minimized) then
...
local function wake_up(self)
    ...
    for _, c in ipairs(added) do
        if not c.floating then
            local wrap = self.client_to_wrapper[c]
            if wrap then
                self.widget:add(wrap)
                wrapper:emit_signal_recursive("added", c)
            else
...
local function handle_hierarchy(context, cr, _hierarchy, wa)
    ...
    assert(cr._matrix.x0 >= wa.x)
    assert(cr._matrix.y0 >= wa.y)
...
local function redraw(_, handler)
    if handler.active then
        ...
        if handler.honor_workarea then
            assert(handler.hierarchy:get_size() <= wa.width)
        end
...
function internal.create_layout(t, l)
    ...
    function handler.arrange(param)
        ...
        local screen_geo   = screen.object.get_bounding_geometry(nil, {
            margins        = handler.honor_gap      ~= false and gap/2 or 0,
            honor_workarea = handler.honor_workarea ~= false,
            honor_padding  = handler.honor_padding  ~= false,
            tag            = handler._tag,
        })

        -- Extra validations to detect off-by-one
        if handler.honor_workarea ~= false then
...
local function register(name, bl, ...)
    ...
    setmetatable(generator, {__call = function(_, t )
        ...
        l_obj.honor_padding  = generator.honor_padding  ~= false
